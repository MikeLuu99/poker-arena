...
- **Starting Chips**: 20 chips per player
...
GOOS=linux GOARCH=amd64 go build -o poker-arena-linux cmd/poker-arena/main.go
GOOS=windows GOARCH=amd64 go build -o poker-arena.exe cmd/poker-arena/main.go
```
...
The project follows Go best practices with clean, modular architecture:

- **`cmd/`**: Application entry points with CLI support
- **`internal/`**: Private application code (game logic, tournaments, server)
- **`pkg/`**: Public models and configuration structures
- **Concurrent Design**: Thread-safe parallel execution with worker pools
- **Clean Separation**: Game logic, AI integration, web serving, and data export are decoupled

### Performance Considerations

- **Parallel Execution**: Configurable concurrency limits prevent resource exhaustion
- **Memory Efficiency**: Games run independently without shared mutable state
- **API Rate Limiting**: Built-in handling for AI provider rate limits and timeouts
- **Graceful Shutdown**: Clean termination with partial result preservation
...
- **Behavioral Studies**: AI decision patterns under different game conditions
- **Competitive Analysis**: Head-to-head AI model performance evaluation

### Entertainment
- **Live Tournaments**: Watch AI models compete in real-time
- **Educational Tool**: Learn poker strategy by observing AI play
- **Streaming Content**: Generate engaging AI vs AI content

### Enterprise Applications
- **Decision Making Research**: Study AI reasoning in competitive environments
- **Model Validation**: Test AI capabilities in structured scenarios
- **Training Data Generation**: Create datasets for reinforcement learning

## Contributing

We welcome contributions to improve Poker Arena! Here's how you can help:
...
- **Game Variants**: Implement different poker variants (Omaha, Stud, etc.)
- **UI Improvements**: Enhance the web interface with new features
- **Performance Optimization**: Improve parallel execution efficiency
- **Data Analysis**: Add new statistical metrics and visualizations
